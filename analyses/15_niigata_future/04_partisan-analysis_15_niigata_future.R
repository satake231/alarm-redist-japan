###############################################################################
# Partisan Analysis for `15_niigata_future`
...
p_dev <- redist.plot.hist(sim_smc_pref_sample, qty = plan_dev, bins = 15) +
  labs(x = "Population Deviation", 
       ...
       title = paste0("Population Deviation - Niigata ", year, " Projection"),
...
p_comp <- redist.plot.hist(sim_smc_pref_sample, qty = comp_edge, bins = 15) +
  labs(x = "Fraction of Edges Kept", 
       y = "Percentage of Plans",
       title = paste0("Compactness - Niigata ", year, " Projection"),
...
p_ruling_jitter <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_share,
                      color_thresh = 0.5) +
  labs(title = paste0("Ruling Coalition Vote Share - Niigata ", year),
...
p_ruling_box <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_share,
  ...
  labs(title = paste0("Ruling Coalition Vote Share Distribution - Niigata ", year),
...
p_scatter1 <- sim_smc_pref_sample %>%
  ...
  labs(title = paste0("LDP vs Komei Vote Share - Niigata ", year),
...
if(exists("redist.plot.distr.custom.color")) {
  ...
  p_custom1 <- redist.plot.distr.custom.color(sim_smc_pref_sample, ruling_share,
    ...
    labs(title = paste0("Ruling Share by District - Niigata ", year),
...
p_opp4_jitter <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_v_opp_coalition_4,
                      color_thresh = 0.5) +
  labs(title = paste0("Ruling vs Opposition (excl. DPP) - Niigata ", year),
...
p_opp_all_jitter <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_v_opp_all,
                      color_thresh = 0.5) +
  labs(title = paste0("Ruling vs All Opposition - Niigata ", year),
...
current_avg_pop <- 2200000 / ndists_old  # Approximate current Niigata population
...
cat("  6 regional cooperation zones defined\n")
cat("  Area 1: Niigata core (central region)\n")
cat("  Area 2: Nagaoka (central-west)\n")
cat("  Area 3: Shibata (northern)\n")
cat("  Area 4: Murakami (far north)\n")
cat("  Area 5: Tsubame-Yahiko\n")
cat("  Area 6: Minamiuonuma (mountainous south)\n")

# Ferry connections impact
cat("\nFerry Connections Impact:\n")
cat("  Sado Island representation maintained via ferry routes\n")
cat("  Awashimaura connectivity to Murakami preserved\n")
cat("  Critical for island community representation\n")
...
cat("3. Assess Nagaoka municipality splitting patterns\n")
cat("4. Review koiki-renkei area balance across reduced districts\n")
cat("5. Monitor Sado Island and Awashimaura ferry connectivity\n")
cat("6. Consider mountainous area representation (Minamiuonuma)\n")
cat("7. Evaluate impact on competitive balance with fewer districts\n")

cat("\nPartisan analysis completed successfully!\n")

# Special note for Niigata decline context
cat("\n=== NIIGATA DECLINE CONTEXT ===\n")
cat("Key factors for 2050 redistricting:\n")
cat("  - Population decline (~20%) driving seat reduction\n")
cat("  - Rural depopulation accelerating\n")
cat("  - Aging society concentration in rural areas\n")
cat("  - Island community representation challenges\n")
cat("  - Ferry connectivity critical for Sado Island\n")
cat("  - Koiki-renkei areas maintain regional cooperation\n")
