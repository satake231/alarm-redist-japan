###############################################################################
# Co-occurrence analysis for `09_tochigi_future`
...
# Find Optimal Plan
# Note that `results_sample` includes the data for future projections (no lh_2022)
...
{
  cooc_ratio[i] <- 1 -
    sum(pref[[paste0("pop_", year)]][relcomp(prefadj[[i]]+1,
                        which(prec_clusters == prec_clusters[i]))] * m_co[i, relcomp(prefadj[[i]]+1,
                                                                                      which(prec_clusters == prec_clusters[i]))])/
    sum(pref[[paste0("pop_", year)]][prefadj[[i]]+1] * m_co[i, prefadj[[i]]+1])
}

# Co-occurrence Plot
# Find the coordinates of major cities
...
cooccurrence_plot <- ggplot() +
  ...
  ggtitle(paste0("Co-occurrence Analysis - ", year, " Projection"))
