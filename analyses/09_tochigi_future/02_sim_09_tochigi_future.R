###############################################################################
# Simulations for `09_tochigi_future`
...
pref_mun_for_processing <- pref_mun %>%
  ...
  rename(pop = !!sym(paste0("pop_", year)))
...
pref_mun_old <- pref_mun_old %>%
  rename(!!sym(paste0("pop_", year)) := pop) %>%
  rename(pop = pop_original)

# Check for missing values in the target population column
pop_col <- paste0("pop_", year)
cat("Missing values in", pop_col, ":", sum(is.na(pref_mun_old[[pop_col]])), "\n")
cat("Zero values in", pop_col, ":", sum(pref_mun_old[[pop_col]] == 0, na.rm = TRUE), "\n")
cat("Total", pop_col, ":", sum(pref_mun_old[[pop_col]], na.rm = TRUE), "\n")
