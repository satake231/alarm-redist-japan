###############################################################################
# Partisan Analysis for `11_saitama_future`
...
p_dev <- redist.plot.hist(sim_smc_pref_sample, qty = plan_dev, bins = 15) +
  labs(x = "Population Deviation", 
       ...
       title = paste0("Population Deviation - Saitama ", year, " Projection"),
...
p_comp <- redist.plot.hist(sim_smc_pref_sample, qty = comp_edge, bins = 15) +
  labs(x = "Fraction of Edges Kept", 
       y = "Percentage of Plans",
       title = paste0("Compactness - Saitama ", year, " Projection"),
...
p_ruling_jitter <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_share,
                      color_thresh = 0.5) +
  labs(title = paste0("Ruling Coalition Vote Share - Saitama ", year),
...
p_ruling_box <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_share,
  ...
  labs(title = paste0("Ruling Coalition Vote Share Distribution - Saitama ", year),
...
p_scatter1 <- sim_smc_pref_sample %>%
  ...
  facet_wrap(~dist_by_ruling_share, ncol = 6) +
  labs(title = paste0("LDP vs Komei Vote Share - Saitama ", year),
...
if(exists("redist.plot.distr.custom.color")) {
  ...
  p_custom1 <- redist.plot.distr.custom.color(sim_smc_pref_sample, ruling_share,
    ...
    labs(title = paste0("Ruling Share by District - Saitama ", year),
...
p_opp4_jitter <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_v_opp_coalition_4,
                      color_thresh = 0.5) +
  labs(title = paste0("Ruling vs Opposition (excl. DPP) - Saitama ", year),
...
p_opp_all_jitter <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_v_opp_all,
                      color_thresh = 0.5) +
  labs(title = paste0("Ruling vs All Opposition - Saitama ", year),
...
cat("  Districts increased: ", ndists_new - ndists_old, " seats\n")
cat("  Percentage increase: ", round((ndists_new - ndists_old) / ndists_old * 100, 1), "%\n")

# Calculate average district population change
# Assume current average population around 7.3M / 16 = ~456k per district
current_avg_pop <- 7300000 / ndists_old 
...
metro_districts <- sum(optimal_plan_data$total_pop > mean(optimal_plan_data$total_pop))
cat("  High-density districts (above average pop): ", metro_districts, "/", ndists_new, "\n")

# 9. Regional Analysis (North-South)
cat("\n=== 9. REGIONAL ANALYSIS ===\n")
cat("North-South Division Analysis:\n")
cat("  Methodology: Partial SMC with 地域振興センター division\n")
cat("  South region includes: 南西部・西部・川越比企地域振興センター\n")
cat("  North region includes: その他（秩父地域含む）\n")

# Estimate regional distribution based on optimal plan
south_codes <- c(11227, 11228, 11229, 11230, 11235, 11245, 11324,  # 南西部
                11208, 11209, 11215, 11225, 11242,                   # 西部
                11201, 11212, 11239, 11241, 11326, 11327)           # 川越比企

# This is a simplified estimation - actual regional assignment would require more detailed mapping
estimated_south_districts <- round(ndists_new * 0.6)  # Roughly 60% for South
estimated_north_districts <- ndists_new - estimated_south_districts

cat("  Estimated South region districts: ", estimated_south_districts, "\n")
cat("  Estimated North region districts: ", estimated_north_districts, "\n")
...
# Save scatter plot with different dimensions (adjusted for 17 districts)
ggsave(here(paste0("data-out/partisan-analysis/", pref_code, "_", pref_name, "_", year, "_ldp_komei_scatter.png")), 
       plot = p_scatter1, width = 15, height = 10, dpi = 300)
...
cat("1. Monitor population projections for metropolitan area growth\n")
cat("2. Consider impact of district increase on political balance\n")
cat("3. Evaluate 入間郡 splitting patterns in final plans\n")
cat("4. Assess competitive balance across increased district count\n")
cat("5. Review さいたま市 municipality splitting patterns\n")
cat("6. Monitor 秩父地域 representation in merged framework\n")
cat("7. Consider Tokyo metropolitan area integration effects\n")

cat("\nPartisan analysis completed successfully!\n")

# Special note for Saitama growth context
cat("\n=== SAITAMA GROWTH CONTEXT ===\n")
cat("Key factors for 2050 redistricting:\n")
cat("  - Population growth (~10%) driving seat increase\n")
cat("  - Metropolitan area expansion into suburban regions\n")
cat("  - Balanced urban-rural representation challenges\n")
cat("  - Integration with Tokyo area political dynamics\n")
cat("  - Partial SMC methodology enabling complex simulations\n")