...
# Assign koiki_renkei area codes (same as original)
...
## Check contiguity (exactly the same process)
...
# TODO (Optional): Add other smaller areas
...
if(nrow(functioning_results) < 5000) {
  warning("Less than 5000 valid plans. Consider increasing nsims.")
}
...
valid_sample <- functioning_results %>%
  filter(draw != "lh_2022") %>%
  pull(draw) %>%
  sample(min(5000, length(.)), replace = FALSE)  # Handle case with < 5000 valid plans
...
# Note: partisan_metrics_japan function should work with HoR data
# as long as the nv_* columns are present in pref_map
...
# `redist_plans` object - with HoR suffix
...
as_tibble(sim_smc_pref_sample) %>%
  ...
  write_csv(here(paste("data-out/stats/",
                      ...
                      "_",
                      as.character(pref_name),
                      "_lh_2022_stats_hor.csv",  # HoR version
                      sep = "")))