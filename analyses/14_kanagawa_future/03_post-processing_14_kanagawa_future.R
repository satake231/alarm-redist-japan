###############################################################################
# Post-processing for `14_kanagawa_future`
# © ALARM Project, May 2023
###############################################################################

cat("=== STARTING KANAGAWA FUTURE POST-PROCESSING ===\n")
cat("Future projection year:", year, "\n")
cat("Prefecture: Kanagawa (", pref_code, ")\n")
cat("District change:", ndists_old, "→", ndists_new, "\n\n")

# TODO Define the koiki-renkei areas (広域連携)
# There are no koiki-renkei areas in Kanagawa

cat("Note: Kanagawa has no koiki-renkei areas\n\n")
...
# Assign koiki_renkei area codes (none for Kanagawa)
koiki_split <- 0
...
cat("Filtering plans with 0 multi-splits...\n")
no_multi <- results %>%
  filter(multi == 0) %>%
  pull(draw)
...
cat("Plans with no multi-splits:", length(no_multi), "\n\n")
...
# Note: Although 川崎市麻生区 has a discontiguous area (川崎市麻生区岡上), 
# both the newly enacted plan and the map used prior to the current redistricting 
# cycle treat that part as if it were contiguous to the main part of 川崎市麻生区.
...
cat("  Total plans with no multi-splits:", nrow(results_no_multi), "\n")
cat("  Valid contiguous plans:", nrow(functioning_results), "\n")
cat("  Invalid plans:", nrow(results_no_multi) - nrow(functioning_results), "\n\n")
...
avg_district_pop_old <- total_pop_2050 / ndists_old
avg_district_pop_new <- total_pop_2050 / ndists_new
...
# Kanagawa-specific context
cat("\nKanagawa-specific context:\n")
cat("  Expected population growth: ~5%\n")
cat("  Metropolitan area (Tokyo adjacent)\n")
cat("  Increased representation needs\n")
cat("  Major cities: Yokohama, Kawasaki, Sagamihara\n")

cat("\nPost-processing completed successfully!\n")
cat("Files saved with year suffix:", year, "\n")
cat("Ready for partisan analysis and co-occurrence analysis.\n")

# Special note for Kanagawa
cat("\n=== KANAGAWA FUTURE REDISTRICTING NOTES ===\n")
cat("1. Population growth accommodated by district increase\n")
cat("2. Complex urban geography in Yokohama and Kawasaki areas\n")
cat("3. Split municipalities reflect high urban density\n")
cat("4. No koiki-renkei areas in Kanagawa\n")
cat("5. Adjacent to Tokyo, part of Greater Tokyo Area\n")