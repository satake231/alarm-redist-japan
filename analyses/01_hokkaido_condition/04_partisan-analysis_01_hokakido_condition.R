...
p_dev <- redist.plot.hist(sim_smc_pref_sample, qty = plan_dev, bins = 15) +
  labs(x = "Population Deviation", 
       ...
       title = paste0("Population Deviation - Hokkaido ", year, " Projection"),
...
p_comp <- redist.plot.hist(sim_smc_pref_sample, qty = comp_edge, bins = 15) +
  labs(x = "Fraction of Edges Kept", 
       y = "Percentage of Plans",
       title = paste0("Compactness - Hokkaido ", year, " Projection"),
...
p_ruling_jitter <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_share,
                      color_thresh = 0.5) +
  labs(title = paste0("Ruling Coalition Vote Share - Hokkaido ", year),
...
p_ruling_box <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_share,
  ...
  labs(title = paste0("Ruling Coalition Vote Share Distribution - Hokkaido ", year),
...
p_scatter1 <- sim_smc_pref_sample %>%
  ...
  facet_wrap(~dist_by_ruling_share, ncol = 5) +
  labs(title = paste0("LDP vs Komei Vote Share - Hokkaido ", year),
...
if(exists("redist.plot.distr.custom.color")) {
  ...
  p_custom1 <- redist.plot.distr.custom.color(sim_smc_pref_sample, ruling_share,
    ...
    labs(title = paste0("Ruling Share by District - Hokkaido ", year),
...
p_opp4_jitter <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_v_opp_coalition_4,
                      color_thresh = 0.5) +
  labs(title = paste0("Ruling vs Opposition (excl. DPP) - Hokkaido ", year),
...
p_opp_all_jitter <- redist.plot.distr_qtys(sim_smc_pref_sample, ruling_v_opp_all,
                      color_thresh = 0.5) +
  labs(title = paste0("Ruling vs All Opposition - Hokkaido ", year),
...
if("gun_split" %in% names(sim_smc_pref_sample)) {
  cat("  振興局 (gun) splits: ", min(sim_smc_pref_sample$gun_split), 
      " to ", max(sim_smc_pref_sample$gun_split), "\n")
  cat("  Mean 振興局 splits: ", round(mean(sim_smc_pref_sample$gun_split), 2), "\n")
}
...
cat("District Reduction Impact:\n")
cat("  Districts reduced: ", ndists_old - ndists_new, " seats\n")
cat("  Percentage reduction: ", round((ndists_old - ndists_new) / ndists_old * 100, 1), "%\n")

# Calculate average district population change
current_avg_pop <- 5224614 # Approximate 2020 Hokkaido population
...
# Save scatter plot with different dimensions
...
summary_stats <- data.frame(
  ...
  value = c(min(sim_smc_pref_sample$plan_dev), max(sim_smc_pref_sample$plan_dev), median(sim_smc_pref_sample$plan_dev),
            ...
            max(optimal_plan_data$total_pop)/min(optimal_plan_data$total_pop), competitive_districts, ndists_old - ndists_new)
)
...
cat("2. Consider impact of district reduction on representation\n")
cat("3. Evaluate 振興局 boundary respect in final plans\n")
cat("4. Assess competitive balance across reduced district count\n")
cat("5. Review Sapporo municipality splitting patterns\n")
